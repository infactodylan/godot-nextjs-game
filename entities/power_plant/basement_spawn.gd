...
const BASEMENT_TOP_ENTRY_Y := 220.0
const BASEMENT_TOP_ENTRY_X := 806.0
...
static func basement_entry_spawn(exit_door: Area2D) -> Vector2:
	...
	if right_edge > 0.0:
		return Vector2(right_edge + PLAYER_HALF_WIDTH + DOOR_CLEARANCE, 220.0)
	return Vector2(exit_door.global_position.x + 64.0, 220.0)
