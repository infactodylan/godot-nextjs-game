...
@onready var village_gate: Area2D = $VillageGate
...
var _village_gate_armed := true
var _village_prompt_open := false
...
func _on_village_gate_entered() -> void:
	if not _village_gate_armed or _village_prompt_open or _phase == GamePhase.MENU:
		return
	_village_prompt_open = true
	hud.show_choice_prompt(
		"Edge of the Wastelands",
		"The village lies to the east — farmland, the courthouse, and safety from the horde.\nReturn to the village?",
		"Return to the Village",
		"Stay in the Wastelands",
		_go_to_village,
		_stay_in_wastelands
	)
...
func _stay_in_wastelands() -> void:
	_village_prompt_open = false


func _on_village_gate_exited() -> void:
	_village_gate_armed = true
	_village_prompt_open = false
